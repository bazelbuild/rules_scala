...
scalapb_proto_library(name, deps, with_grpc, with_java, with_flat_package, with_single_line_to_string, scalac_jvm_flags, runtime)
```
...
<table class="table table-condensed table-bordered table-params">
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        <p><code>List of labels, required</code></p>
        <p>List of <code>proto_library</code> dependencies for this target</p>
      </td>
    </tr>
    <tr>
    ...
    <tr>
      ...
      <td>
        <p><code>List of labels; optional</code></p>
        <p>List of <code>*_library</code> that the generated code is compiled against (ScalaPB dependencies are added implicitly)<p>
        <p>If <code>with_java</code> is used then <code>java_proto_library</code> for which conversions are generated must be supplied</p>
      </td>
    </tr>
  </tbody>
</table>
