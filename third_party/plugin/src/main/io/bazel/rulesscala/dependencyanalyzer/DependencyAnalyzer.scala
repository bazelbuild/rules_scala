...
class DependencyAnalyzer(val global: Global) extends Plugin with LocateClassFile with GlobalHelpers {
  ...
  val dependencies: mutable.Set[File] = mutable.Set[File]()
  private val localToNonLocalClass = new LocalToNonLocalClass[DependencyAnalyzer.this.global.type](global)
  ...
  private object Component extends PluginComponent {
    ...
    override def newPhase(prev: Phase): StdPhase = new StdPhase(prev) {
      ...
      private def warnOnIndirectTargetsFoundIn(usedJars: Set[File]) = {
        for (usedJar <- usedJars;
             usedJarPath = usedJar.getPath;
      ...
      override def apply(unit: CompilationUnit): Unit = {
        ...
        warnOnIndirectTargetsFoundIn(dependencies.toSet)
      }
    }
