...
def _handle_jar_stamping(ctx, jar):
    source_file = jar
    if ctx.label.package != jar.dirname:
        symlink = ctx.actions.declare_file(jar.basename)
        source_file = symlink
        ctx.actions.run(
            inputs = [jar],
            outputs = [symlink],
            # XXX need to make this relative to deal with remote caching etc
            executable = "ln",
            arguments = [jar.path, symlink.path],
        )
...
def _scala_import_impl(ctx):
    ...
    current_target_stamped_compile_jars = [
        _handle_jar_stamping(ctx, jar)
        for jar in current_target_compile_jars
    ]
    current_jars = depset(current_target_stamped_compile_jars)
    ...
    if current_target_stamped_compile_jars:
        current_target_providers = [_new_java_info(ctx, jar) for jar in current_target_stamped_compile_jars]
    else:
