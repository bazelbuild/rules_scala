...
def scala_test_suite(name, srcs = [], deps = [], runtime_deps = [], data = [], resources = [],
    ...
    for test_file in srcs:
        n = "suite_test_%s_%s_test" % (name, test_file)
        scala_test(n, srcs = [test_file], deps = deps, runtime_deps = runtime_deps, resources=resources, scalacopts=scalacopts, jvm_flags=jvm_flags, visibility=visibility, size=size)
        ts.append(n)
...
def scala_library_suite(name,
    ...
    for src_file in srcs:
        ...
        ts.append(n)
    scala_library(name = name, exports = exports + ts)
