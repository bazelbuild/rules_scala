...
package bazel_test_runner.junitrunner.java.com.google.testing.junit.runner.util;

import javax.annotation.Nullable;

/** JUnit runner integration code for TestIntegration. */
public class TestIntegrationsRunnerIntegration {
  private static final ThreadLocal<TestIntegrationsExporter.Callback> callbackForThread =
      new ThreadLocal<TestIntegrationsExporter.Callback>() {
        @Override
        protected TestIntegrationsExporter.Callback initialValue() {
  ...
  public static TestIntegrationsExporter.Callback setTestCaseForThread(@Nullable TestIntegrationsExporter.Callback callback) {
    TestIntegrationsExporter.Callback previousCallback = callbackForThread.get();
    if (callback == null) {
  ...
  static TestIntegrationsExporter.Callback getCallbackForThread() {
    // TODO(bazel-team): This won't work if the test is running in a different thread from the test
    // runner.
    return callbackForThread.get();
  }

  private static class NoOpCallback implements TestIntegrationsExporter.Callback {
