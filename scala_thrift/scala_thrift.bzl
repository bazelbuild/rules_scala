...
ScalaThriftAspectInfo = provider(fields = [
    "thrift_info",
    "src_jars",
    "output_files",
    "java_info",
...
ScalaThriftInfo = provider(fields = [
    "aspect_info",
])

ScalaThriftImport = provider(fields = [
    "java_info",
    "thrift_info",
])

def merge_scala_thrift_aspect_info(scrooges):
    return ScalaThriftAspectInfo(
...
def _compile_to_scala(ctx, label, compile_thrifts, include_thrifts, jar_output):
    ...
    ctx.actions.run(
        executable = ctx.executable._pluck_scala_thrift,
        inputs = compile_thrifts + include_thrifts + [argfile],
        outputs = [jar_output],
        mnemonic = "ScalaThriftRule",
...
# or a scala_thrift_library. Each thrift_library will be one scrooge
# invocation assuming it has some sources.
def _scala_thrift_aspect_impl(target, ctx):
    if ScalaThriftImport in target:
        target_import = target[ScalaThriftImport]
        target_ti = target_import.thrift_info
        deps = [target_import.java_info]
        transitive_ti = target_ti
    else:
        target_ti = target[ThriftInfo]
        deps = [d[ScalaThriftAspectInfo].java_info for d in ctx.rule.attr.deps]
        transitive_ti = merge_thrift_infos(
            [
                d[ScalaThriftAspectInfo].thrift_info
...
scala_thrift_aspect = aspect(
    implementation = _scala_thrift_aspect_impl,
    attr_aspects = ["deps"],
    attrs = {
        "_pluck_scala_thrift": attr.label(
            executable = True,
            cfg = "host",
...
def _scala_thrift_library_impl(ctx):
    aspect_info = merge_scala_thrift_aspect_info(
        [dep[ScalaThriftAspectInfo] for dep in ctx.attr.deps],
    )
    if ctx.attr.exports:
    ...
    return [
        all_java,
        ScalaThriftInfo(aspect_info = aspect_info),
        DefaultInfo(files = aspect_info.output_files),
    ]

scala_thrift_library = rule(
    implementation = _scala_thrift_library_impl,
    attrs = {
        "deps": attr.label_list(aspects = [scala_thrift_aspect]),
        "exports": attr.label_list(providers = [JavaInfo]),
    },
    provides = [DefaultInfo, ScalaThriftInfo, JavaInfo],
)

def _scala_thrift_import_impl(ctx):
    jars_jis = [
        JavaInfo(
    ...
    return [java_info, ti, ScalaThriftImport(java_info = java_info, thrift_info = ti)]

# Allows you to consume thrifts and compiled jars from external repos
scala_thrift_import = rule(
    implementation = _scala_thrift_import_impl,
    attrs = {
    ...
    provides = [ThriftInfo, JavaInfo, ScalaThriftImport],
)
