...
class ScalaImportNeverLinkTest extends SpecificationWithJUnit {
  "neverlinked scala_import" should {
    "not be available in runtime" in {
      //ScalaImportNeverLink case class is packaged in scala_import_never_link.jar. Since the scala_import target
      ...
      ScalaImportNeverLink() must throwA[NoClassDefFoundError]("scalarules/test/scala_import/nl/ScalaImportNeverLink")
    }
  }
